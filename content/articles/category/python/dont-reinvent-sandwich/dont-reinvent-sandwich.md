Title: Don't Reinvent Sandwich: Decorator
Date: 2017-06-25 22:00
Category: python
Tags: python, 
Slug: dont-reinvent-sandwich
...
由於本文相當長，為了閱讀上的方便我將它分成前後兩篇，分別著重在 Python 的裝飾器和環境管理器，這是本文的上半部分，我們會討論 Python 的裝飾器。

* 延伸閱讀：[Don't Reinvent Sandwich 2: Context Manager](http://dokelung.me/category/python/dont-reinvent-sandwich-2)
...
def clock(func):
    # wraps 會將 func 的屬性抄到 new_func 裡讓 new_func 看起來跟 func 一模一樣
    @wraps(func)
    def new_func(*args, **kwargs):
        """裝飾過的函數"""
        t0 = time.time()
...
def myroute(url):
    def deco(view_func):
        ...
        return view_func # 把原函數當成修飾過的函數即可，這次的重點在於註冊，而非改變原函數行為
    return deco
```
